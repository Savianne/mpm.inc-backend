...
$query = htmlentities($data['query'], ENT_QUOTES, 'UTF-8');
$thread_id = htmlentities($data['thread_id'], ENT_QUOTES, 'UTF-8');
$exp_date = htmlentities($data['exp_date'], ENT_QUOTES, 'UTF-8');
// $status = 'Waiting for sender varification';
                    ...
                    <table role="presentation" style="width:100%;border-collapse:collapse;border-spacing:0;">
                        <tr style="width: 100%;padding: 0;margin: 0;text-align: center;" >
                            <td style="margin: 0;padding:15px 0">
                                <p> You sent us a query from our website <a href="https://mpministry.site">https://mpministry.site</a> </p>
                            </td>
                        </tr>
                        <tr style="width: 100%;padding: 0;margin: 0;text-align: center;" >
                            <td style="margin: 0;padding:15px 0">
                                <form style="display: inline-block; min-width: 350px; width: 30%; padding: 20px 0; background-color: #dad4d41c; box-shadow: 0 0 5px 1px #d0c1c1; margin: 15px 0 0 0">
                                    ...
                                    <input style="min-width: 300px; width: 80%; height: 40px; margin-bottom: 15px; border: none;outline: 1.5px solid rgba(0, 94, 170, 0.479);" value="$subject" disabled></input>
                                    <textarea style="min-width: 300px; width: 80%; height: 80px; margin-bottom: 15px; border: none;outline: 1.5px solid rgba(0, 94, 170, 0.479);" disabled >$query</textarea>
                                </form>
                            </td>
                        </tr>
                        <tr style="width: 100%;padding: 0;margin: 0;text-align: center;" >
                            <td style="margin: 0;padding:15px 0">
                                <p style="margin: 0 0 5px 0;">If this was made by you? please click <a href="http://localhost:82/verify_query.php?thread_id=$thread_id">YES</a> or else <a href="http://localhost:82/cancel_query.php?thread_id=$thread_id" style="color: red">NO</a>.</p>
                            </td>
                        </tr>
                        <tr style="width: 100%;padding: 0;margin: 0;text-align: center;" >
                            <td style="margin: 0;padding:15px 0">
...
$param = [
    "subject" => "[[Query Verification]] Macedonian Personal Ministry",
    "body" => $body,
];
...
if(send_email_to_user($email, $param)) {
    ...
    $sql = "INSERT INTO queries (thread_id, full_name, email, subject, query, exp_date) VALUES (?, ?, ?, ?, ?, ?)";
    if($stmt = $db->db->prepare($sql)){
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("ssssss", $thread_id, $full_name, $email, $subject, $query, $exp_date);
        $stmt->execute();
        // Close statement
        $stmt->close();
